...
class AbstractTrainer(object):
    ...
    def __init__(self, config_A, config_B, config_C, model_A, model_B, model_C):
        ...
        self.config_B = config_C
        self.model_A = model_A
        self.model_B = model_B
        self.model_B = model_C
...
class FedtrainTrainer(Trainer):
    ...
    def __init__(self, config_A, config_B, config_C, model_A, model_B, model_C):
        super(FedtrainTrainer, self).__init__(config_A, config_B, config_C, model_A, model_B, model_C)
    ...
    def fedtrain(self, train_data_A, train_data_B, verbose=True, show_progress=False):
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            train_loss_C, item_prompt_C = self._train_epoch_C(train_data_C, epoch_idx, show_progress=show_progress)
            ...
            self.train_loss_dict_C[epoch_idx] = sum(train_loss_C) if isinstance(train_loss_C, tuple) else train_loss_C
            ...
            cluster_prompt_C = cluster_prompt[item_prompt_C.shape[0]:].to(self.device)
            self.model_A.global_prompt.data[1:] = cluster_prompt_A
            self.model_B.global_prompt.data[1:] = cluster_prompt_B
            self.model_C.global_prompt.data[1:] = cluster_prompt_C
            if (epoch_idx + 1) % self.save_step == 0:
                ...
                self.save_pretrained_model_C(epoch_idx, saved_model_file_C)
                ...
                update_output_C = set_color('Saving current', 'blue') + ': %s' % saved_model_file_C
                if verbose:
                    self.logger.info(update_output_A)
                    self.logger.info(update_output_B)
                    self.logger.info(update_output_C)
