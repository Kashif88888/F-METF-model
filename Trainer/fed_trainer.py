...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config_A, config_B, config_C, model_A, model_B, model_C):
        super(Trainer, self).__init__(config_A, config_B, config_C, model_A, model_B, model_C)
        ...
        self.optimizer_C = self._build_optimizer_C()
    ...
    def _train_epoch_A(self, train_data, epoch_idx, cluster_prompt_A, loss_func=None, show_progress=False):
        self.model_A.train()
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_A.calculate_loss(interaction, epoch_idx)
            # if epoch_idx != 0:
            #     # losses += torch.norm(self.model_A.item_prompt.data - cluster_prompt_A)
            #     losses += 0.05 * ((self.model_A.item_prompt.data - cluster_prompt_A).norm(2))
        ...
        # with torch.no_grad():
        #     # prompt_A = self.model_A.get_prompt().clone().detach()
        #     prompt_A = self.model_A.item_prompt.clone().detach()
        return total_loss, self.model_A.item_prompt.clone().detach()

    def _train_epoch_B(self, train_data, epoch_idx, cluster_prompt_B, loss_func=None, show_progress=False):
        self.model_B.train()
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = self.model_B.calculate_loss(interaction, epoch_idx)
            # if epoch_idx != 0:
            #     # losses += torch.norm(self.model_B.item_prompt.data - cluster_prompt_B)
            #     losses += 0.05 * ((self.model_B.item_prompt.data - cluster_prompt_B).norm(2))
            if isinstance(losses, tuple):
                loss = sum(losses)
                loss_tuple = tuple(per_loss.item() for per_loss in losses)
                total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('Loss: ' + str(losses.item()), 'yellow'))
            # prompt_B = self.model_B.get_prompt().clone().detach()
        return total_loss, self.model_B.item_prompt.clone().detach()
...
class FedtrainTrainer(Trainer):
    ...
    def fedtrain(self, train_data_A, train_data_B, verbose=True, show_progress=False):
        ...
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            train_loss_A, item_prompt_A = self._train_epoch_A(train_data_A, epoch_idx, copy.deepcopy(cluster_prompt_A), show_progress=show_progress)
            train_loss_B, item_prompt_B = self._train_epoch_B(train_data_B, epoch_idx, copy.deepcopy(cluster_prompt_B), show_progress=show_progress)
            ...
            all_prompt = torch.cat((item_prompt_A, item_prompt_B), dim=0).cpu()
            k = 30
            kmeans = KMeans(n_clusters=k, random_state=106)
            clusters = kmeans.fit(all_prompt.numpy())
            cluster_centers = torch.Tensor(clusters.cluster_centers_)
            cluster_label = clusters.labels_
            # cluster_aggregate_vectors = torch.zeros(k, all_prompt.size(1))
            # cluster_item_counts = torch.zeros(k)
            # for i in range(all_prompt.size(0)):
            #     cluster_index = clusters[i]
            #     cluster_aggregate_vectors[cluster_index] += all_prompt[i]
            #     cluster_item_counts[cluster_index] += 1
            # for i in range(k):
            #     if cluster_item_counts[i] > 0:
            #         cluster_aggregate_vectors[i] /= cluster_item_counts[i]
            cluster_prompt = torch.zeros_like(all_prompt)
            for i in range(all_prompt.size(0)):
                cluster_prompt[i] = cluster_centers[cluster_label[i]]
            ...
            # self.model_A.global_prompt.data = cluster_prompt_A
            # self.model_B.global_prompt.data = cluster_prompt_B
