...
class FMETFLayer(nn.Module):
    """FMETF-enhanced Adaptor
    """
    def __init__(self, n_exps, layers, dropout=0.0, noise=True):
        super(FMETFLayer, self).__init__()
...
class MLTRec(SASRec):
    def __init__(self, config, dataset):
        ...
        self.moe_adaptor = FMETFLayer(
            config['n_exps'],
            config['adaptor_layers'],
            config['adaptor_dropout_prob']
        )
        self.global_adapter = FMETFLayer(
            config['n_exps'],
            config['adaptor_layers'],
            config['adaptor_dropout_prob']
        )
        filter_layer = FilterLayer(self.hidden_size, self.max_seq_length, config['filter_dropout_prob'])
        self.filter_layer = nn.ModuleList([copy.deepcopy(filter_layer) for _ in range(config['filter_layer'])])
    ...
    def forward(self, item_seq, item_emb, item_seq_len):
        ...
        for layer_module in self.filter_layer:
            input_emb = layer_module(input_emb)
    ...
    def calculate_loss(self, interaction):
        ...
        fed_prompt = self.global_FMETF(self.global_prompt)
        id_gate = torch.sigmoid(self.id_gating(self.item_embedding.weight))
        test_item_emb = id_gate * self.item_embedding.weight + text_emb
        prompt_list = self.filter_prompt(self.global_FMETF(self.global_prompt[item_seq]))
