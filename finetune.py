...
from model.FMETF_model import MLTRec

from data.dataset import ECGDataset
from data.dataset import EEGataset
...
def finetune(dataset, pretrained_file, plm, fix_enc=True, **kwargs):
    ...
    props = ['props/FMETF.yaml', 'props/finetune.yaml']
    print(props)
    ...
    if pretrained_file != '':
        ...
        logger.info(f'Transfer [{checkpoint["config"]["dataset"]}] -> [{dataset}] -> [{dataset}]')
