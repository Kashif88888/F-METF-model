...
from utils import check_path, set_device, load_plm, ECG_Hospital_A
...
def generate_item_embedding(args, item_text_list, item2index, tokenizer, model, word_drop_ratio=-1):
    ...
    while start < len(order_texts):
        ...
        if args.emb_type == 'CLS':
            cls_output = [hidden_states[:, 0, ].detach().cpu() for hidden_states in outputs.hidden_states[1:]]
            embeddings.append(torch.cat(cls_output, dim=-1))
...
def parse_args():
    ...
    parser.add_argument('--output_path', type=str, default='../bert/')
